...
function SiparisKarti({ s, onDurumDegis, onSil, onAsamaDegis, onDetayAc, onYukle }) {
  ...
  const kategoriLabel = [...ANA_KATEGORILER].find(k=>k.key===s.kategori_ana)?.label || Object.values(ALT_KATEGORILER).flat().find(i=>i.key===s.kategori)?.label;
  ...
  const handleDosyaYukle = async (e) => {
    ...
    try { await dosyaApi.yukle(sid, fd); toast('Dosya yüklendi ✓'); onYukle(); } catch { toast('Yükleme hatası','error'); } finally { e.target.value=''; }
  };
  return (
    <div className={`siparis-card ${s.oncelik||''}`} style={{position:'relative'}}>
      <div style={{flex:1,minWidth:0}}>
        ...
        <div className="siparis-meta" style={{gap:10,flexWrap:'wrap'}}>
          {kaynak && <span style={{display:'flex',alignItems:'center',gap:4}}><i className={`ti ${kaynak.icon}`} style={{fontSize:14}}/>{kaynak.label}</span>}
          {kategoriLabel && <><span style={{color:'var(--text3)'}}>·</span><span>{kategoriLabel}</span></>}
          {s.kargo?.takip_no && <><span style={{color:'var(--text3)'}}>·</span><span style={{display:'flex',alignItems:'center',gap:4}}><i className="ti ti-truck" style={{fontSize:13}}/>{s.kargo.takip_no}</span></>}
        </div>
        {s.urunler?.length>0&&s.urunler[0]?.ad&&(<div className="siparis-meta" style={{marginTop:4}}><i className="ti ti-notes" style={{fontSize:13}}/>{s.urunler.map((u,i)=><span key={i}>{u.adet>1?`${u.adet}× `:''}{u.ad}</span>)}</div>)}
        {dl.text&&(<div className="deadline-row" style={{marginTop:6}}><i className="ti ti-calendar-due" style={{fontSize:14,color:dl.cls==='acil'?'var(--red)':dl.cls==='yakin'?'var(--amber)':'var(--green)'}}/><span className={`deadline-text ${dl.cls}`}>{dl.text}</span>{s.teslim_tarihi&&<span style={{fontSize:12,color:'var(--text3)'}}>({new Date(s.teslim_tarihi+'T00:00:00').toLocaleDateString('tr-TR')})</span>}</div>)}
        {toplamAsama>0&&(
          <div style={{marginTop:8}}>
            <button onClick={()=>setAsamaAcik(a=>!a)} style={{background:'none',border:'none',color:'var(--text3)',fontSize:12,cursor:'pointer',padding:0,display:'flex',alignItems:'center',gap:6}}>
              <i className={`ti ti-chevron-${asamaAcik?'up':'down'}`} style={{fontSize:12}}/>Aşamalar ({tamamlananAsama}/{toplamAsama})
              <div style={{width:60,height:3,background:'var(--border)',borderRadius:2,overflow:'hidden'}}><div style={{height:'100%',background:'var(--accent)',width:`${(tamamlananAsama/toplamAsama)*100}%`,transition:'width 0.3s'}}/></div>
            </button>
            {asamaAcik&&(<div style={{display:'flex',flexWrap:'wrap',gap:6,marginTop:8}}>{s.asamalar.map((asama,idx)=>{const info=ASAMA_LABEL[asama.key]||{};return(<button key={idx} onClick={()=>onAsamaDegis(sid,idx)} style={{display:'flex',alignItems:'center',gap:5,padding:'5px 10px',borderRadius:'var(--radius-sm)',fontSize:12,cursor:'pointer',border:`1px solid ${asama.tamamlandi?(info.renk||'var(--green)'):'var(--border)'}`,background:asama.tamamlandi?`${info.renk||'var(--green)'}22`:'var(--bg3)',color:asama.tamamlandi?(info.renk||'var(--green)'):'var(--text2)'}}><i className={`ti ${asama.tamamlandi?'ti-circle-check':(info.icon||'ti-circle')}`} style={{fontSize:13}}/>{asama.label}</button>);})}</div>)}
          </div>
        )}
      </div>
      <div style={{display:'flex',flexDirection:'column',alignItems:'flex-end',gap:8,flexShrink:0}}>
        <select className="status-select" value={s.durum} onChange={e=>onDurumDegis(sid,e.target.value)}>
          <option value="bekliyor">Bekliyor</option><option value="hazirlaniyor">Hazırlanıyor</option>
          <option value="hazir">Hazır</option><option value="kargoda">Kargoda</option><option value="teslim_edildi">Teslim Edildi</option>
        </select>
        <div style={{display:'flex',gap:5,position:'relative'}}>
          <div style={{position:'relative'}}>
            <button className="btn-icon" onClick={()=>togglePanel('odeme')} title="Ödeme" style={{color:s.odeme?.odendi?'var(--green)':s.odeme?.tutar?'var(--amber)':'var(--text2)'}}>
...
export default function SiparislerPage() {
  ...
  const yukle = useCallback(async () => {
    setYukleniyor(true);
    try { const r = await siparisApi.getAll(); setData(r.data); } catch { toast('Yüklenemedi','error'); } finally { setYukleniyor(false); }
  }, []);
  ...
  const renderGrup=(baslik,emoji,liste)=>{
    ...
    return (<><div className="section-divider">{emoji} {baslik} ({liste.length})</div>{liste.map(s=><SiparisKarti key={s._id||s.id} s={s} onDurumDegis={handleDurumDegis} onSil={handleSil} onAsamaDegis={handleAsamaDegis} onDetayAc={setDetaySiparis} onYukle={yukle}/>)}</>);
  };
  return (
    <div>
      <div className="page-header">
        ...
        <div style={{display:'flex',gap:8}}>
          <button className="btn btn-secondary" onClick={async()=>{try{const r=await shopierApi.cek();toast(r.data.mesaj);yukle();}catch(e){toast(e.response?.data?.hata||'Shopier hatası','error');}}}>
            <i className="ti ti-refresh"/>Shopier'den Çek
          </button>
      ...
      <div className="filter-row">
        {[['hepsi','Tümü'],['acil','🔴 Acil'],['yakin','🟡 Bu hafta'],['bekliyor','Bekliyor'],['hazirlaniyor','Hazırlanıyor'],['hazir','Hazır'],['kargoda','Kargoda'],['odeme',odemeBekleyen>0?`💳 Ödeme (${odemeBekleyen})`:'💳 Ödeme']].map(([key,label])=>(
      ...
      : filtre==='hepsi' ? <>{renderGrup('Acil — bugün ve yarın','🔴',aciller)}{renderGrup('Bu hafta','🟡',yakinlar)}{renderGrup('Sonraki hafta ve sonrası','🟢',normaller)}</>
      : filtrelenmis.map(s=><SiparisKarti key={s._id||s.id} s={s} onDurumDegis={handleDurumDegis} onSil={handleSil} onAsamaDegis={handleAsamaDegis} onDetayAc={setDetaySiparis} onYukle={yukle}/>)}
      {data.teslim_edilen?.length>0&&(
        <div style={{marginTop:24}}>
          ...
          {arsivAcik&&data.teslim_edilen.map(s=>(
            <div key={s._id||s.id} className="siparis-card" style={{opacity:0.55,marginTop:8,borderLeftColor:'var(--border)'}}>
              <div>
                ...
                {s.teslim_edildi_tarihi&&<div style={{fontSize:12,color:'var(--text3)',marginTop:4}}>{new Date(s.teslim_edildi_tarihi).toLocaleDateString('tr-TR')}</div>}
              </div>
              <div style={{display:'flex',gap:5}}>
                <button className="btn-icon" onClick={()=>setDetaySiparis(s)} title="Detay"><i className="ti ti-dots" style={{fontSize:15}}/></button>
