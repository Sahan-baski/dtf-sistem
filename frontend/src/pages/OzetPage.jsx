...
function hesaplaPeriyot(siparisler, periyot) {
  ...
  if (periyot === '6hafta' || periyot === '3ay' || periyot === '6ay') {
    ...
    for (let i=haftaSayisi-1; i>=0; i--) {
      ...
      const haftalikler = siparisler.filter(s=>{ const t=new Date(s.createdAt); return t>=baslangic&&t<=bitis; });
      const ciro = haftalikler.reduce((t,s)=>t+(s.odeme?.tutar||0),0);
      const tahsil = haftalikler.filter(s=>s.odeme?.odendi).reduce((t,s)=>t+(s.odeme?.tutar||0),0);
      items.push({ label:`${baslangic.getDate()}/${baslangic.getMonth()+1}`, ciro, tahsil });
    }
  } else {
    for (let i=11; i>=0; i--) {
      const d = new Date(now); d.setDate(1); d.setMonth(d.getMonth()-i);
      const baslangic = new Date(d); baslangic.setHours(0,0,0,0);
      const bitis = new Date(baslangic); bitis.setMonth(bitis.getMonth()+1); bitis.setDate(0); bitis.setHours(23,59,59,999);
      const aylikler = siparisler.filter(s=>{ const t=new Date(s.createdAt); return t>=baslangic&&t<=bitis; });
      const ciro = aylikler.reduce((t,s)=>t+(s.odeme?.tutar||0),0);
      const tahsil = aylikler.filter(s=>s.odeme?.odendi).reduce((t,s)=>t+(s.odeme?.tutar||0),0);
      items.push({ label:baslangic.toLocaleDateString('tr-TR',{month:'short'}), ciro, tahsil });
    }
  }
  return items;
}
...
export default function OzetPage({ onSiparislerGit }) {
  ...
  useEffect(() => {
    ...
    Promise.all([ozetApi.get(), siparisApi.getAll()]).then(([oz, sip]) => {
      setOzet(oz.data);
  ...
  return (
    <div>
      ...
      {/* Bugün teslim */}
      <div className="card" style={{ marginBottom:16, borderLeft:'4px solid var(--red)', padding:'18px 20px' }}>
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:12 }}>
          <div style={{ fontSize:15, fontWeight:700, color:'var(--red)' }}>
            <i className="ti ti-calendar-due" style={{ marginRight:8 }}/>
            Bugün Teslim ({bugunSiparisler.length})
          </div>
          <button className="btn btn-sm btn-secondary" onClick={onSiparislerGit}>Tüm Siparişler <i className="ti ti-arrow-right"/></button>
        </div>
        {bugunSiparisler.length===0 ? (
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(2,1fr)', gap:8 }}>
            {bugunSiparisler.map(s=>(
              <div key={s._id||s.id} style={{ display:'flex', alignItems:'center', gap:10, padding:'9px 12px', background:'var(--bg3)', borderRadius:'var(--r-xs)', border:'1px solid rgba(239,68,68,0.2)' }}>
                ...
                <div style={{ flex:1, minWidth:0 }}>
                  <div style={{ fontWeight:600, fontSize:13, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{s.musteri_adi} {s.musteri_soyadi}</div>
        ...
        {gecikmisList.length>0 && (
          <div style={{ marginTop:12, padding:'9px 12px', background:'rgba(239,68,68,0.07)', borderRadius:'var(--r-xs)', border:'1px solid rgba(239,68,68,0.2)', fontSize:13, color:'#fca5a5' }}>
            <i className="ti ti-alert-triangle" style={{ marginRight:6 }}/>{gecikmisList.length} gecikmiş sipariş — acil düzenleme gerekiyor!
          </div>
        )}
      </div>
      ...
      <div className="card" style={{ marginBottom:20, padding:'20px 20px 16px' }}>
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:18 }}>
          <div>
            <div style={{ fontSize:15, fontWeight:700 }}><i className="ti ti-chart-bar" style={{ color:'var(--indigo)', marginRight:8 }}/>Ciro Gidişatı</div>
          </div>
          <div style={{ display:'flex', gap:4 }}>
            {PERIYOT.map(p=>(
              <button key={p.key} onClick={()=>setPeriyot(p.key)}
                style={{ padding:'5px 12px', borderRadius:20, border:`1px solid ${periyot===p.key?'var(--indigo)':'var(--border)'}`, background:periyot===p.key?'rgba(99,102,241,0.15)':'var(--bg3)', color:periyot===p.key?'var(--indigo)':'var(--text2)', fontSize:12, cursor:'pointer' }}>
                {p.label}
              </button>
            ))}
          </div>
        ...
        <div style={{ display:'flex', gap:16, marginTop:8, justifyContent:'center' }}>
          <span style={{ display:'flex', alignItems:'center', gap:5, fontSize:12, color:'var(--text2)' }}>
            <span style={{ width:10, height:10, borderRadius:2, background:'rgba(99,102,241,0.6)', display:'inline-block' }}/>Toplam Ciro
          </span>
          <span style={{ display:'flex', alignItems:'center', gap:5, fontSize:12, color:'var(--text2)' }}>
            <span style={{ width:10, height:10, borderRadius:2, background:'rgba(16,185,129,0.7)', display:'inline-block' }}/>Tahsil Edilen
          </span>
        </div>
      </div>
      ...
      {ozet && (
        <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:12 }}>
          {[
            ...
            { label:'Ödeme Bekleyen',val:`${(ozet.bekleyenTahsilat||0).toLocaleString('tr-TR')}₺`, color:'var(--amber)', icon:'ti-wallet' },
