...
function StatKart({ sayi, label, renk, icon, alt }) {
  return (
...
export default function OzetPage() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ozetApi.get().then(r => setOzet(r.data)).catch(() => {});
    }, 30000);
  ...
  return (
    <div>
      <div className="page-header">
        <div>
          <div className="page-title">🏠 Günlük Özet</div>
          <div style={{ fontSize: 13, color: 'var(--text3)', marginTop: 2 }}>{bugun}</div>
        </div>
      </div>
      ...
      <div className="stats-row" style={{ gridTemplateColumns: 'repeat(3, 1fr)', marginBottom: 24 }}>
        <StatKart
          sayi={`${(ozet.buAyGelir || 0).toLocaleString('tr-TR')}₺`}
          label="Bu Ay Tahsil Edildi"
          renk="var(--green)" icon="ti-coin" />
        <StatKart
          sayi={`${(ozet.bekleyenTahsilat || 0).toLocaleString('tr-TR')}₺`}
          label="Bekleyen Tahsilat"
          renk="var(--amber)" icon="ti-clock" />
        <StatKart
          sayi={ozet.odemeBekleyen}
          label="Ödeme Bekleyen Sipariş"
          renk="var(--red)" icon="ti-credit-card-off" />
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
        ...
        <div>
          ...
          {ozet.bugunTeslimListesi?.length === 0 ? (
            <div style={{ color: 'var(--text3)', fontSize: 13, padding: '12px 0' }}>Bugün teslim yok 🎉</div>
        ...
        <div>
          ...
          {ozet.gecikmisList?.length === 0 ? (
            <div style={{ color: 'var(--text3)', fontSize: 13, padding: '12px 0' }}>Gecikmiş sipariş yok ✓</div>
            ...
            <div key={s.id} className="card" style={{ padding: '10px 14px', marginBottom: 8, borderLeft: '3px solid var(--red)' }}>
              ...
              <div style={{ fontSize: 12, color: 'var(--text2)', marginTop: 2 }}>
                #{s.siparis_no} · {s.teslim_tarihi ? new Date(s.teslim_tarihi+'T00:00:00').toLocaleDateString('tr-TR') : '?'}
              </div>
            </div>
          ))}
        </div>
      ...
      {ozet.asamaBekleyen?.length > 0 && (
        <div style={{ marginTop: 8 }}>
          <div className="section-divider">🔧 Devam eden üretim aşamaları</div>
          {ozet.asamaBekleyen.map(s => (
            <div key={s.id} className="card" style={{ padding: '10px 14px', marginBottom: 8, display: 'flex', alignItems: 'center', gap: 12 }}>
              <div style={{ flex: 1 }}>
                <span style={{ fontWeight: 600, fontSize: 14 }}>{s.musteri}</span>
                <span style={{ fontSize: 12, color: 'var(--text3)', marginLeft: 8 }}>#{s.siparis_no}</span>
              </div>
              <div style={{ fontSize: 12, background: 'rgba(79,126,248,0.15)', color: 'var(--accent)', padding: '3px 10px', borderRadius: 20, border: '1px solid rgba(79,126,248,0.3)' }}>
                {s.bekleyen_asama}
              </div>
              {s.teslim_tarihi && (
