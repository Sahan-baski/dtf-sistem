...
export default function GorevlerPage() {
  ...
  const yukle = useCallback(async () => {
    try {
      const res = await gorevApi.getAll();
  ...
  const handleEkle = async (e) => {
    ...
    try {
      ...
      toast('Görev eklenemedi', 'error');
    }
  };

  const handleToggle = async (id) => {
    try {
      await gorevApi.toggleTamamla(id);
      yukle();
    } catch {
    ...
    <div>
      <div className="page-header">
        ...
        <div style={{ color: 'var(--text2)', fontSize: 13 }}>
          {aktif.length} bekliyor · {tamamlandi.length} tamamlandı
        </div>
      </div>
        ...
        <form onSubmit={handleEkle}>
          <div style={{ display: 'flex', gap: 8, marginBottom: detayAcik ? 12 : 0 }}>
            <input
              ...
              placeholder="Yeni görev ekle... (Örn: Kataloğa 5 yeni ürün eklenecek)"
              value={yeniBaslik}
              onChange={e => setYeniBaslik(e.target.value)}
              onFocus={() => setDetayAcik(true)}
              style={{ flex: 1 }}
            />
      ...
      {aktif.length === 0 ? (
        ...
        <>
          {['yuksek', 'normal', 'dusuk'].map(onc => {
            const grup = aktif.filter(g => g.oncelik === onc);
            ...
            return (
              <div key={onc}>
                ...
                {grup.map(g => (
                  <div key={g.id} className="gorev-card">
                    <div
                      className={`gorev-checkbox ${g.tamamlandi ? 'checked' : ''}`}
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      {g.aciklama && (
                        <div style={{ fontSize: 12, color: 'var(--text3)', marginTop: 2 }}>{g.aciklama}</div>
                      )}
                    </div>
                    <span className={`badge ${o.cls}`}>{o.label}</span>
                    <button className="btn-icon" onClick={() => handleSil(g.id)}>
      ...
      {tamamlandi.length > 0 && (
        <>
          ...
          {tamamlandi.map(g => (
            <div key={g.id} className="gorev-card tamamlandi">
              <div
                className="gorev-checkbox checked"
                onClick={() => handleToggle(g.id)}
